...
uint256 constant FIVE_PERCENT = 5e16; // {1} 0.05
uint256 constant TWENTY_PERCENT = 20e16; // {1} 0.2
uint256 constant TWENTY_FIVE_PERCENT = 25e16; // {1} 0.25
uint256 constant FORTY_FIVE_PERCENT = 45e16; // {1} 0.45
uint256 constant FIFTY_PERCENT = 50e16; // {1} 0.5
uint256 constant NINETY_FIVE_PERCENT = 95e16; // {1} 0.95

uint256 constant MAX_EXP = 6502287e18; // {1} (1000000/999999)^6502287 = ~666.6667
uint256 constant BASE = 999999e12; // {1} (999999/1000000)
uint256 constant ONE_POINT_FIVE = 150e16; // {1} 1.5
 ...
 */
contract FolioDutchTrade is ITrade, Versioned {
    ...
    uint256 public bestPrice; // {buyTok/sellTok} The best plausible price based on oracle data
    uint256 public worstPrice; // {buyTok/sellTok} The worst plausible price based on oracle data
    ...
    function lot() public view returns (uint256) {
        return sellAmount.shiftl(int8(sell.decimals()), Math.Rounding.Down);
    }
    ...
    function init(
        ...
        require(
            prices.sellLow != 0 && prices.sellHigh != 0 && prices.sellHigh < type(uint256).max / 1000,
            "bad sell pricing"
        );
        require(
            prices.buyLow != 0 && prices.buyHigh != 0 && prices.buyHigh < type(uint256).max / 1000,
            "bad buy pricing"
        );

        // broker = IBroker(msg.sender);
        origin = origin_;
        sell = IERC20Metadata(sell_);
        buy = IERC20Metadata(buy_);
        ...
        uint256 _worstPrice = prices.sellLow.mulDiv(
            1e18 - origin.maxTradeSlippage(),
            prices.buyHigh,
            Math.Rounding.Down
        );
        uint256 _bestPrice = prices.sellHigh.divrnd(prices.buyLow, Math.Rounding.Up); // no additional slippage
    ...
    function bid() external returns (uint256 amountIn) {
        ...
        // {buyTok/sellTok}
        uint256 price = _price(uint48(block.timestamp)); // enforces auction ongoing
    ...
    function bidWithCallback(bytes calldata data) external returns (uint256 amountIn) {
        ...
        // // reportViolation if auction cleared in geometric phase
        // if (price > bestPrice.mul(ONE_POINT_FIVE, Math.Rounding.Up)) {
        //     broker.reportViolation();
        // }
        ...
        require(amountIn <= buy.balanceOf(address(this)) - balanceBefore, "insufficient buy tokens");
    ...
    function _price(uint48 timestamp) private view returns (uint256) {
        ...
        uint256 progression = (timestamp - _startTime) / (_endTime - _startTime); // {1}

        // Fast geometric decay -- 0%-20% of auction
        if (progression < TWENTY_PERCENT) {
            uint256 exp = MAX_EXP.mulDiv(TWENTY_PERCENT - progression, TWENTY_PERCENT, Math.Rounding.Zero);
            ...
            return bestPrice.mulDiv(ONE_POINT_FIVE, BASE.powu(uint48(exp), Math.Rounding.Up));
            ...
            uint256 _bestPrice = bestPrice; // gas savings
            // {buyTok/sellTok} = {buyTok/sellTok} * {1}
            uint256 highPrice = _bestPrice.mul(ONE_POINT_FIVE, Math.Rounding.Up);
            return highPrice - (highPrice - _bestPrice).mulDiv(progression - TWENTY_PERCENT, TWENTY_FIVE_PERCENT);
            ...
            uint256 _bestPrice = bestPrice; // gas savings
            // {buyTok/sellTok} = {buyTok/sellTok} * {1}
            return _bestPrice - (_bestPrice - worstPrice).mulDiv(progression - FORTY_FIVE_PERCENT, FIFTY_PERCENT);
        }
    ...
    /// @return {qBuyTok} The amount of buy tokens required to purchase the lot
    function _bidAmount(uint256 price) public view returns (uint256) {
        // {qBuyTok} = {sellTok} * {buyTok/sellTok} * {qBuyTok/buyTok}
        return sellAmount.mul(price, Math.Rounding.Up).shiftl(int8(buy.decimals()), Math.Rounding.Up);
    }
}
