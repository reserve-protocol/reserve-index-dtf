    ...
    Versioned
{
    ...
    /// @dev The other Auction fields are included in the autogenerated `auctions()` getter
    /// @return Whether the token is in the given auction
    function tokenInAuction(uint256 auctionId, address token) external view returns (bool) {
        return auctions[auctionId].inAuction[token];
    }

    /// Get the currently ongoing rebalance
    /// @dev The other Rebalance fields are included in the autogenerated `rebalance()` getter
