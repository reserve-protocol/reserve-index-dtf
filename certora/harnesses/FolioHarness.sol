...
import { DEFAULT_ADMIN_ROLE, MAX_TVL_FEE, MAX_FEE_RECIPIENTS, MAX_TTL, MAX_LIMIT, MAX_WEIGHT, MAX_TOKEN_BUY_AMOUNT, MAX_TOKEN_PRICE, MAX_TOKEN_PRICE_RANGE, MIN_MINT_FEE } from "../../contracts/utils/Constants.sol";
...
contract FolioHarness is Folio {
    ...
    function _isTokenInSurplus(
        ...
        return
            Math.mulDiv(Math.mulDiv(limitH, weightH, 1e18, Math.Rounding.Ceil), totalShares, 1e27, Math.Rounding.Ceil) <
            currentBalance;
    }
    ...
    function getMAX_MINT_FEE() public pure returns (uint256) {
        return MAX_MINT_FEE;
    }
    function getMAX_FOLIO_FEE() public pure returns (uint256) {
        return MAX_FOLIO_FEE;
    }
    function getMIN_AUCTION_LENGTH() public pure returns (uint256) {
        return MIN_AUCTION_LENGTH;
    }
    function getMAX_AUCTION_LENGTH() public pure returns (uint256) {
        return MAX_AUCTION_LENGTH;
    }
