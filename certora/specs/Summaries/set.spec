...
hook Sstore currentContract.basket._inner._values.length uint256 newLength {
    ghostLength = newLength;
}
// Store hook to synchronize ghostValues array with set._inner._values.
hook Sstore currentContract.basket._inner._values[INDEX uint256 index] bytes32 newValue {
    ghostValues[index] = newValue;
}
// Store hook to synchronize ghostIndexes array with set._inner._indexes.
hook Sstore currentContract.basket._inner._positions[KEY bytes32 value] uint256 newIndex {
...
hook Sload uint256 length currentContract.basket._inner._values.length {
    require ghostLength == length;
}
hook Sload bytes32 value currentContract.basket._inner._values[INDEX uint256 index] {
    require ghostValues[index] == value;
}
hook Sload uint256 index currentContract.basket._inner._positions[KEY bytes32 value] {
