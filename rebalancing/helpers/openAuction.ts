...
export const openAuction = (
  ...
  if (
    ...
    (spotPrice > initialPrices.start * 50n || startPrice > initialPrices.start * 100n || spotPrice < initialPrices.end)
  ) {
  ...
  const wholeSellLimit = targetBasket[x].mul(sharesValue).div(prices[x]).div(supply);
