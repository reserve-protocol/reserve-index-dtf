...
function _parseScientific(s: string): bigint {
  ...
  const frac_part = match.groups.frac_part ? BigInt(match.groups.frac_part) : BigInt(0);
  let exponent = match.groups.exponent ? BigInt(match.groups.exponent) : BigInt(0);
  ...
  const positive_output =
    exponent >= BigInt(0) ? int_part * BigInt(10) ** BigInt(exponent) : int_part / BigInt(10) ** BigInt(-exponent);
