...
export const getDustAmount = (
  ...
  const price = bn(new Decimal(_price).mul(D27d).div(new Decimal(`1e${decimals}`)));
...
export const getCurrentBasket = (bals: bigint[], decimals: bigint[], _prices: number[]): bigint[] => {
  // D27{USD/tok} = {USD/wholeTok} * D27 / {tok/wholeTok}
  const prices = _prices.map((a, i) => bn(new Decimal(a).mul(D27d).div(new Decimal(`1e${decimals[i]}`))));
