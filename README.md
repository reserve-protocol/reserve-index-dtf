...
For mint fees, the receiver still receives `shares - totalFeeShares`, including a deduction for the self-fee shares. The self-fee shares initially remain in the effective supply, keeping the mint exchange rate neutral apart from asset-transfer rounding. Starting at each 24-hour boundary, pending mint self-fee shares are virtually burned (handed out to holders) linearly for `FOLIO_FEE_HANDOUT_PERIOD` at a maximum rate of half the minimum mint fee per nominal 12-second block (1.5 bps). New self-fees accrued during an open handout window can join its remaining time, and excess pending shares above the maximum daily appreciation of roll over to later days. Accrued mint self-fees do not account for the ongoing TVL fee supply expansion that occurs before they are burned, but this error is small.

Under a four-minute handout period at 1.5 basis points (0.015%) per block, at most 30 basis points (0.30%) of appreciation can be handed out per day. This is the maximum sustained self-fee appreciation the Folio can support: average accrual above this capacity creates an indefinitely growing backlog, effectively postponing the excess appreciation forever. This is a known limitation that is acceptable under realistic conditions, equivalent to a 3x appreciation event per year, solely from mint self-fees. If the daily maximum rate is temporarily exceeded, the 30 basis point maximum serves as a throttling mechanism to cap the incentive for potential short-term entrants.
