...
In order to enable CowSwap on upgrade, set the SwapperRegistry (`setSwapperRegistry`) to a non-zero address. As part of a pre-hook of a CowSwap order, a swap contract can be deployed via `openSwap()` that holds a swap balance and validates EIP-1271 orders, isolating the swap from the rest of the Folio.
...
3. post-hook: call `swap.close()` (not required, but please do it as a nicety ideally)
