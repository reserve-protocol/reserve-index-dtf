...
Folios support rebalancing trades via Dutch Auction over an exponential decay curve between two prices. Control flow over the trade is shared between two parties, with a `TRADE_PROPOSER` approving trades in advance and a `AUCTION_LAUNCHER` opening them, optionally providing some amount of additional detail.
...
1. `DEFAULT_ADMIN_ROLE`
   ...
   - Can configure the `TRADE_PROPOSER`/ `AUCTION_LAUNCHER`
