...
TVL fees and mint self-fee handouts use independent checkpoints. `lastFolioFeePoke` advances on every poke, while `lastPoke` advances only when daily TVL fee time is accounted. On upgrade, a zeroed `lastFolioFeePoke` is initialized by the first poke without granting historical handout capacity.
